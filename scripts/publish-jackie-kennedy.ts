/**
 * One-off: create the Jackie Kennedy jewellery story, scheduled for its
 * 6 August 2026 launch date. Not published immediately — publishedAt is set
 ...
 * on/after that date.
 * Run: npx tsx scripts/publish-jackie-kennedy.ts
 */
...
const SLUG = 'jackie-kennedy-jewellery'
const DOC_ID = `article-${SLUG}`
const LAUNCH_DATE = '2026-08-06'

async function main() {
  const file = join(__dirname, '../content/65-jackie-kennedy-jewellery.md')
  ...
  const doc = {
    ...
    heroImageAlt: 'A triple-strand pearl necklace laid against dark fabric.',
    body,
