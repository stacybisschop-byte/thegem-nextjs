/**
 * One-off: create the best tennis bracelets (US) guide, scheduled for its
 * 20 August 2026 launch date. Not published immediately — publishedAt is set
 ...
 * on/after that date.
 * Run: npx tsx scripts/publish-best-tennis-bracelets-us.ts
 */
...
const SLUG = 'best-tennis-bracelets-us'
const DOC_ID = `article-${SLUG}`
const LAUNCH_DATE = '2026-08-20'

async function main() {
  const file = join(__dirname, '../content/70-best-tennis-bracelets-us.md')
  ...
  const doc = {
    ...
    heroImageUrl: 'https://images.unsplash.com/photo-1573408301185-9146fe634ad0?w=2400&q=80&fit=crop&crop=entropy',
    heroImageAlt: 'A diamond line bracelet laid against a dark background.',
    body,
