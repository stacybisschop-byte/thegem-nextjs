/**
 * One-off: create the signet rings for women guide, scheduled for its
 * 8 August 2026 launch date. Not published immediately — publishedAt is set
 ...
 * on/after that date.
 * Run: npx tsx scripts/publish-signet-rings-women.ts
 */
...
const SLUG = 'signet-rings-women'
const DOC_ID = `article-${SLUG}`
const LAUNCH_DATE = '2026-08-08'

async function main() {
  const file = join(__dirname, '../content/64-signet-rings-women.md')
  ...
  const doc = {
    ...
    heroImageAlt: 'A gold signet ring on a pale surface.',
    body,
