/**
 * One-off: create Catbird vs Jennie Kwon. publishedAt is 20 Sep 2026 (already
 * past), so the article goes live immediately (published: true).
 * Featured at order -4.75: slot 2 of the homepage Latest grid ("Also new this week"),
 * behind the hero (-5) and ahead of the Autumn Edit (-4.5).
 * Run: npx tsx scripts/publish-catbird-vs-jennie-kwon.ts
 */
...
const SLUG = 'catbird-vs-jennie-kwon'
const DOC_ID = `article-${SLUG}`

const HERO = {
  path: 'C:/Users/stacy/Downloads/catbird-vs-jennie-kwon.png',
  alt: 'The Catbird shop sign above the Jennie Kwon Designs logo, two American makers of solid 14ct gold compared.',
}

async function main() {
  const file = join(__dirname, '../content/79-catbird-vs-jennie-kwon.md')
  ...
  const heroAsset = await client.assets.upload('image', readFileSync(HERO.path), {
    filename: 'catbird-vs-jennie-kwon.png',
  })
